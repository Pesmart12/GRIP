...
// Central-difference d(phi_i)/dq for one contact, by re-running real
// detection at perturbed configurations.
Eigen::RowVector3d FiniteDifferencePhiJacobian(const RigidBodyState& state, const BodyShape& shape, const HalfPlane& plane, std::size_t contact_index) {
  const std::function<Eigen::Matrix<double, 1, 1>(const Eigen::Vector3d&)> phi_of_q = [&](const Eigen::Vector3d& q) {
    RigidBodyState perturbed = state;
    perturbed.q = q;
    Eigen::Matrix<double, 1, 1> out;
    out(0) = detect_contacts_body(perturbed, shape, plane)[contact_index].phi;
    return out;
  };
  return testutil::CentralDifferenceJacobian<1, 3>(phi_of_q, state.q);
}

TEST(ContactJacobians, MatchCentralFiniteDifference) {
  // Rotated to a generic angle so dphi/dtheta is nonzero for every
  // vertex -- an axis-aligned box would leave some of them at values
  // that a sign error could still reproduce.
  ...
  for (std::size_t i = 0; i < analytic.size(); ++i) {
    const Eigen::RowVector3d fd = FiniteDifferencePhiJacobian(state, shape, plane, i);
    for (int j = 0; j < 3; ++j) {
...
TEST(ContactJacobians, MatchCentralFiniteDifferenceOnTiltedPlane) {
  // Non-axis-aligned normal, so both translation components of
  // dphi/dq are nontrivial rather than 0 and 1.
  HalfPlane plane;
...
TEST(ContactJacobians, TranslationBlockIsThePlaneNormal) {
  // phi is linear in the center position, so dphi/dx and dphi/dy are
  // exactly the plane normal for every vertex, at any orientation --
...
TEST(ContactJacobians, RotationDerivativeVanishesAtExtremalCorner) {
  ...
  // height, so phi is stationary in theta there and dphi/dtheta must be
  // zero. Checks the J*(p-c) term specifically -- a wrong sign or a
  // missing rotation would still be nonzero here.
  RigidBodyState state;
